...
export const ActivityLogModal: React.FC<ActivityLogModalProps> = ({ isOpen, onClose, logs: propLogs }) => {
  ...
  useEffect(() => {
    if (propLogs) {
      ...
      try {
        const stored = localStorage.getItem('tracex_custody_logs');
        if (stored) {
              ...
              {filteredLogs.length > 0 ? (
                ...
                <tr>
                  <td colSpan={6} className="py-8 text-center text-slate-400 text-xs">
                    No chain-of-custody activities recorded yet. Ingest media to create the initial cryptographic custody ledger.
                  </td>
                </tr>
              )}
            </tbody>
