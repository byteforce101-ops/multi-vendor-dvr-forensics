"""backend/tests/test_tracex_forensic_detector.py — Tests for Pure TraceX Forensic Detector & Reconstructor."""
...
def test_tracex_forensic_detector_vehicle_classification():
...
def test_tracex_forensic_detector_centroid_tracking():
...
def test_tracex_reconstructor_trajectory_and_loitering():
    """Verify trajectory event reconstruction, direction tracking, and loitering identification."""
    reconstructor = TraceXForensicReconstructor(loitering_min_seconds=2.0, loitering_max_radius=50.0)

    # Simulate tracked person moving Eastbound
    obs_east = [
        (0.0, [TraceXForensicDetection(class_name="person", confidence=0.85, bbox=(10, 10, 30, 60), track_id=1, velocity=(20.0, 0.0))]),
        (1.0, [TraceXForensicDetection(class_name="person", confidence=0.85, bbox=(30, 10, 50, 60), track_id=1, velocity=(20.0, 0.0))]),
        (2.0, [TraceXForensicDetection(class_name="person", confidence=0.85, bbox=(50, 10, 70, 60), track_id=1, velocity=(20.0, 0.0))]),
    ]
    ...
    obs_loiter = [
        (0.0, [TraceXForensicDetection(class_name="person", confidence=0.85, bbox=(100, 100, 120, 150), track_id=2, velocity=(0.0, 0.0))]),
        (1.5, [TraceXForensicDetection(class_name="person", confidence=0.85, bbox=(105, 102, 125, 152), track_id=2, velocity=(2.0, 1.0))]),
        (3.5, [TraceXForensicDetection(class_name="person", confidence=0.85, bbox=(102, 98, 122, 148), track_id=2, velocity=(-1.0, 0.0))]),
    ]
...
def test_video_analysis_service_with_pure_forensic():
    """Verify VideoAnalysisService runs end-to-end with 100% pure forensic detector engine."""
    service = VideoAnalysisService(
        detector_engine="forensic",
    ...
    for r in ai_results:
        assert r.source in ("forensic", "tracex")
...
def test_temporal_confirmation_suppresses_transient_noise():
    ...
    detector = TraceXForensicDetector(min_hits=2)
    detector.reset_tracks()
