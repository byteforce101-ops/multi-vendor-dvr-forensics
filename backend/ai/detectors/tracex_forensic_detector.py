"""TraceX Pure Forensic Object Detector & Tracker.
...
1. Pedestrian HOG feature detector for people.
2. Haar Cascades for full body and upper body poses.
3. MOG2 Motion & Morphometric Aspect-Ratio Classifier for vehicles, cyclists, and moving objects.
4. Native Centroid Object Tracker with predictive velocity across video frames.
5. Strict forensic whitelisting (zero household false positives).
"""
...
@dataclass
class TraceXForensicDetection:
    """Represents a forensic detection from the TraceX pure forensic detector."""
...
class TraceXForensicDetector:
    """TraceX pure forensic multi-stage detector and tracker."""
    ...
    def detect_frame(
        ...
        frame: np.ndarray,
        fps: float = 2.0,
    ) -> list[TraceXForensicDetection]:
        """Detect and track all forensic entities in a single frame using pure forensic vision."""
        ...
        candidates: list[TraceXForensicDetection] = []
                    ...
                    if norm_conf >= self.confidence_threshold:
                        ...
                        candidates.append(
                            TraceXForensicDetection(
                                ...
                                attributes={"detector": "forensic_hog"},
                            )
                        )
            except Exception as exc:
                ...
                for bx, by, bw, bh in bodies:
                    candidates.append(
                        TraceXForensicDetection(
                            ...
                            attributes={"detector": "forensic_haar_body"},
                        )
                    )
            except Exception as exc:
                    ...
                    if overlaps_person:
                        candidates.append(
                            TraceXForensicDetection(
                                ...
                                attributes={"detector": "forensic_motion_person_overlap"},
                            )
                        )
                    else:
                        candidates.append(
                            TraceXForensicDetection(
                                ...
                                attributes={"detector": "forensic_motion_cluster"},
                            )
                        )
    ...
    def _apply_nms(
        self,
        detections: list[TraceXForensicDetection],
        iou_threshold: float = 0.45,
    ) -> list[TraceXForensicDetection]:
        """Apply Non-Maximum Suppression to eliminate overlapping duplicates."""
        if not detections:
            return []

        def _sort_score(d: TraceXForensicDetection) -> float:
            score = d.confidence
        ...
        kept: list[TraceXForensicDetection] = []
    ...
    def _update_tracks(
        ...
        cost_candidates: list[tuple[float, int, int]] = []
